...
def createCarpetPlotYAngles(y_angle_array, hour_in_month, H, C, E, E_HCL):  
    ...
    plt.subplot(2,3,4)
    carpetPlotMask(X = y_angle_array['SolarEnergy'], z_min = -45, z_max= 45, title = '(d) PV Supply Optimization', hour_in_month = hour_in_month)
    plt.xlabel("Month of the Year",size=14)
    plt.ylabel("Hour of the Day",size=14)
    
    plt.subplot(2,3,5)
    carpetPlotMask(X = y_angle_array[E_HCL], z_min = -45, z_max= 45, title = '(e) Building Demand Optimization', hour_in_month = hour_in_month)
    plt.xlabel("Month of the Year",size=14)    
