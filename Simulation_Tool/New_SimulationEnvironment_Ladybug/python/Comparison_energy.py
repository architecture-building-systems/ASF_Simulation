...
def EnergyConsumption_plot(monthlyData, FloorArea, Title):
	...
	for i in range(len(monthlyData['E_total'][u'H'])):
		...
		PV_prod[month] += monthlyData['E_total'][u'PV'][i]/FloorArea #kWh/m2
	...
	if max (Cooling_elec)!= 0 : 
		if max(Heating_elec)!=0 and max(Heating_fossil)!=0:
			...
			ax.bar(months, PV_prod, width, label='PV production')
...
def importData(path):
	...
	with open(os.path.join(path, 'MonthlyElecFlow.json')) as json_file:
		monthlyFlowElec = json.load(json_file)


	with open(os.path.join(path, 'monthlyData.json')) as json_file:
...
def init():
	paths = {}
	FolderNameWith = 'Results_ZH_Classic_WWR027_threshold_ArchA' #Results_ZH_Classic_WWR027_threshold_ArchA, Results_ZH_Classic_WWR031_threshold_ArchB, Results_ZH_Classic_WWR040_threshold_ArchC, Results_ZH_ASF_WWR027_ArchA, Results_ZH_ASF_WWR031_ArchB, Results_ZH_ASF_WWR04_ArchC
	FolderNameWithout = 'Results_ZH_without_WWR027_ArchA' #Results_ZH_Classic_WWR027_threshold_ArchA, Results_ZH_Classic_WWR031_threshold_ArchB, Results_ZH_Classic_WWR040_threshold_ArchC, Results_ZH_ASF_WWR027_ArchA, Results_ZH_ASF_WWR031_ArchB, Results_ZH_ASF_WWR04_ArchC
	...
	paths['sim_results_with'] = os.path.join(paths['results'], FolderNameWith)
	paths['sim_results_without'] = os.path.join(paths['results'], FolderNameWithout)
	paths['sim_Figures_with'] = os.path.join(paths['sim_results_with'], 'Figures')
	paths['sim_Figures_without'] = os.path.join(paths['sim_results_without'], 'Figures')

	if not os.path.isdir(paths['sim_results_with']):
...
if __name__=='__main__':
	...
	monthlyFlowElec, monthlyData = importData(paths['sim_results_with'])
	title = "With"

	figA, consumption_df_with = EnergyConsumption_plot(monthlyData, roomFloorArea, title) #kWh/m2
	fig.update({'figA': figA})
	#SaveFig(figA, 'EnergyConsumption', paths['sim_Figures_with'])
	
	consumption_df_with.to_csv(os.path.join(paths['sim_results_with'], 'consumption.csv'))

	monthlyFlowElec, monthlyData = importData(paths['sim_results_without'])
	title = "Without"

	figB, consumption_df_without = EnergyConsumption_plot(monthlyData, roomFloorArea, title) #kWh/m2
	fig.update({'figB': figB})
	#SaveFig(figA, 'EnergyConsumption', paths['sim_Figures_without'])
	
	consumption_df_without.to_csv(os.path.join(paths['sim_results_without'], 'consumption.csv'))
