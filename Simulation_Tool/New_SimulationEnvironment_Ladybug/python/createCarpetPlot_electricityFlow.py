...
def createCarpetPlot (monthlyData, roomFloorArea):
    ...
    z_max = round(max(monthlyData[u'consumed_elec'])/roomFloorArea, 3)
    z_min = -round(max(monthlyData[u'self_prod'])/roomFloorArea, 3)
    ...
    plt.subplot(2,2,1)
    carpetPlot(X = monthlyData[u'consumed_elec'], z_min = z_min, z_max= z_max, title = '(a) el. demande', roomFloorArea = roomFloorArea)
    plt.ylabel("Hour of the Day",size=16)
    
    plt.subplot(2,2,2)
    carpetPlot(X = -np.asarray(monthlyData[u'self_prod']), z_min = z_min, z_max= z_max, title = '(b) PV Supply', roomFloorArea = roomFloorArea)
    
    plt.subplot(2,2,3)
    carpetPlot(X = -np.asarray(monthlyData[u'export_elec']), z_min = z_min, z_max= z_max, title = '(c) el. exported', roomFloorArea = roomFloorArea)
    
    
    plt.subplot(2,2,4)
    carpetPlot(X = monthlyData[u'import_elec'], z_min = z_min, z_max= z_max, title = '(d) el. imported', roomFloorArea = roomFloorArea)
    ...
    tick = np.arange(z_min, z_max, 0.1)
