...
class ASF_Simulation(object):
    ...
    def __init__(self,
            ...
            'COP_H': 3, 'COP_C':3},
            SimulationOptions= 
            {'setBackTempH' : 4.,'setBackTempC' : 4., 'Occupancy' : 'Occupancy_COM.csv','ActuationEnergy' : False, 'Temp_start' : 20}):
...
#            f.write(json.dumps(FolderName))    
    ...
    def setPaths(self):
        ...
        self.paths['radiation_results'] = os.path.join(self.paths['main'],'radiation_results_' + self.FolderName['DataNamePV'])
        self.paths['radiation_wall'] = os.path.join(self.paths['main'],  'radiation_wall_' + self.FolderName['DataNamePV'])
    ...
    def runRadiationCalculation(self):
        ...
        self.HourlyRadiation = CalculateRadiationData(SimulationPeriode = self.SimulationPeriod,
                                                ...
                                                paths = self.paths, 
                                                DataNamePV = self.FolderName['DataNamePV'],
                                                DataNameWin = self.FolderName['DataNameWin'])
        ...
        if self.PanelData['numberHorizontal'] == 0 and self.PanelData['numberVertical'] == 0:
            ...
            if not os.path.isfile(os.path.join(self.paths['PV'], 'HourlyPV_electricity_results_' + self.FolderName['DataNamePV'] + '.npy')): 
                ...
                self.PV_electricity_results = np.load(os.path.join(self.paths['PV'], 'HourlyPV_electricity_results_' + self.FolderName['DataNamePV'] + '.npy')).item()
                self.PV_detailed_results = np.load(os.path.join(self.paths['PV'], 'HourlyPV_detailed_results_' + self.FolderName['DataNamePV'] + '.npy')).item()
                print '\nLadyBug data loaded from Folder:'
                print 'radiation_results_' + self.FolderName['DataNamePV']  
        ...
        for monthi in range(self.SimulationPeriod['FromMonth'], self.SimulationPeriod['ToMonth'] + 1):
           for day in range(self.SimulationPeriod['FromDay'], self.SimulationPeriod['ToDay'] + 1):
                for hour in range(self.SimulationPeriod['FromHour'], self.SimulationPeriod['ToHour'] + 1):        
                    ...
                    HOY = calcHOY(month= self.monthi,day = self.day, hour= self.hour)
    ...
    def runBuildingSimulation(self):
        ...
        for optimizationType in self.optimization_Types:
            ...
            self.hourlyData[optimizationType], self.ResultsBuildingSimulation[optimizationType], self.UncomfortableH[optimizationType], self.TotalHOY = RC_Model (
                                                                          optimization_type = self.optimizationType, 
                                                                          paths = self.paths,
                                                                          ...
                                                                          setBackTemp = self.setBackTemp,
                                                                          occupancy = self.occupancy,
                                                                          Q_human = Q_human, 
                                                                          start = self.start, 
                                                                          end = self.end, 
                                                                          Temp_start = self.Temp_start,
                                                                          SimulationPeriod = self.SimulationPeriod
                                                                          )
            ...
            self.BestKey[optimizationType], self.x_angles[optimizationType], self.y_angles[optimizationType],  self.TotalHourlyData[optimizationType] = prepareAngles(
    ...
    def SaveResults(self):
        ...
        if self.Save == True: 
            ...
            self.paths['result']= os.path.join(self.paths['result_folder'], 'Results_' + self.geoLocation + '_date_' + self.now + '_name_' + self.SimulationData['DataName'])
            ...
            for ii in self.optimization_Types:
                ...
                #self.fig[ii].savefig(os.path.join(self.paths['result'], 'figure_' + ii +  '.png'))
                #np.save(os.path.join(os.path.join(self.paths['result'], 'BuildingSimulation_'+ ii + '.npy')), self.ResultsBuildingSimulation[ii])
