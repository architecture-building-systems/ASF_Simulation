...
from NoClusters import NoClusters
...
for HOY in range(start,end):
    ...
    for x_angle in XANGLES:
        for y_angle in YANGLES:
            ...
            #GH kann files nicht lesen, da der name geändert hat
            else:
                with open('outputASFangles.json','w') as f:
                    outputASFangles = NoClusters()
                    
                    f.write(json.dumps(outputASFangles))
                
                for i in range(0,panel_data['NoClusters']):
                    while not os.path.exists(paths['radiation_results']+'\\RadiationResults' +'_' + str(HOY) + '_' + str(x_angle) + '_' + str(y_angle)+ '.csv'):
                        time.sleep(1)
                    else:
                        print 'hallo'
