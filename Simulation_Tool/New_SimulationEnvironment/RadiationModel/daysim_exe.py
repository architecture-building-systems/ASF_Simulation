...
def geometry2radiance(arad, ageometry_table, ainput_path, project_path, project_folder, STLFolder):
    # parameters for the radiance
    
    
    #project_folder = r'C:\Users\Assistenz\Desktop\Mauro\radiation_visualization'
    
    ainput_path = os.path.join(project_folder, STLFolder) # set one general folder with all stl files
...
def calc_radiation(input_path = None, project_path = None, geometry_table_name = None, weatherfile_path = None, sen_list=None, sensor_geometries_name=None, rad_params = None, project_folder = None, STLFolder = None):

    # =============================== parameters =============================== #
    results_path = os.path.join(project_path, 'output')
    #input_path = os.path.join(project_path, 'input')
    
    with open(os.path.join(input_path,'weatherFile.json'),'w') as f:
            f.write(json.dumps(weatherfile_path))
    # =============================== Preface =============================== #
    
    
    rad_path = os.path.join(project_folder, 'rad_data')
            
    #rad = py2radiance.Rad(os.path.join(input_path, 'base.rad'), os.path.join(input_path, 'py2radiance_data'))
    rad = py2radiance.Rad(os.path.join(rad_path, 'base.rad'), os.path.join(rad_path, 'py2radiance_data'))

    # =============================== Import =============================== #
    geometry_table = pd.read_csv(os.path.join(input_path, geometry_table_name+".csv"), index_col='name')

    # =============================== Simulation =============================== #
    geometry2radiance(rad, geometry_table, input_path, project_path, project_folder, STLFolder)
...
if __name__ == '__main__': 
    ...
    for x_angle in XANGLES:
        for y_angle in YANGLES:  
            ...
            project_folder = os.path.join(project_folder, ProjectSubFolder)
            ...
            pathFile = os.path.join(os.path.join(os.path.join(project_folder, '2ASF_' + str(x_angle) + '_' + str(y_angle)),'output\Window'), 'res\Window.csv')
            print pathFile
            if not os.path.isfile(pathFile):
                ...
                #option to chose    
                calc_radiation(input_path, project_path, geometry_table_name, weatherfile_path, sen_list=sen_list, rad_params = rad_params, project_folder = project_folder, STLFolder = STLFolder)
