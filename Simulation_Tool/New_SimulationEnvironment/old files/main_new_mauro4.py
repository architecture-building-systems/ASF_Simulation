...
for jj in Rad_sum:
    ...
    RadiationBuilding_HOY[j] = jj - PV_results[j]
    j += 1

print RadiationBuilding_HOY

a = "PV_results_HOY_" + str(HOY)
...
# add system path, so that all files are available:
...
sys.path.insert(0, 'C:\Users\Assistenz\Desktop\Mauro\ASF_Simulation\Simulation_Tool\New_SimulationEnvironment\RC_BuildingSimulator-master\simulator\data')                                        
...
print '\nOptimal angle combination(X-angle,Y-angle):', combinationAngles[max(PV_results_HOY, key=lambda i: PV_results_HOY[i])]
