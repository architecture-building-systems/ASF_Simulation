...
def asf_electricity_production(createPlots=False,
    ...
    for x_angle in XANGLES:
        for y_angle in YANGLES:
                fileName =  SimulationData['SubFolder']+ '_' + str(x_angle) + '_' + str(y_angle) + '_' + str(start) + '_' + str(end-1) + '_' + str(MatDict['ASF'])
                DaySimData[str(x_angle) + str(y_angle)] = np.load(os.path.join(paths['saveDaySim'], fileName + '.npy')).item()
