...
@author: Johannes Hofer, Jeremias Schmidli
"""
from pylab import *
import time
import scipy
...
def diode_equation(y,Iphi,Io,a,CC,x,AA,BB2):
    return y - Iphi + Io * (e**(a/CC*(x + y * AA))-1) + (x + y * AA)/BB2
...
def diode_equation_eval(fitparams=None, x=None, modparams=None, ins=None, temp=None):
    ...
    # Rp_0 = 10*BB;
    Rp_0 = 4 * BB
    Rp = BB + (Rp_0 - BB) * e**(-5.5 * (phi / phi_ref))
    BB2 = Rp

    # photocurrent and diode saturation current
    Iphi_ref = Isc + DD * (exp(Isc * AA * a / CC) - 1) + Isc * AA / BB
    Iphi = (phi / phi_ref) * (Iphi_ref + muIsc * (Tc - Tc_ref))
    Io = DD * (Tc / Tc_ref) ** 3 * exp(b / CC * (1 / Tc_ref - 1 / Tc))
...
save('curr_model_submod_lookup',curr_model_submod_lookup)
